...
impl HtmlParser
{
    ...
    fn create_element(&mut self, tag: &str, attributes: Vec<Attribute>) -> Node
    {
    ...
    fn contain_in_stack(&mut self, elment_kind: ElementKind) -> bool
    {
        for i in 0..self.stack_of_open_elements.len()
        {
            if self.stack_of_open_elements[i].borrow().element_kind() == Some(elment_kind)
            {
                return true;
            }
        }
                ...
                InsertionMode::BeforeHtml =>
                    {
                        ...
                        self.insert_elemtn("html", Vec::new());
                        self.mode = InsertionMode::BeforeHead;
                        continue;
                    }
                InsertionMode::BeforeHead =>
                    {
                        match token
                        {
                            ...
                            Some(HtmlToken::StartTag
                                 ...
                                 }) =>
                                {
                                    if tag == "head"
                                    {
                                        self.insert_elment(tag, attributes.to_vec());
                        ...
                        self.insert_elment("head", Vec::new());
                        self.mode = InsertionMode::InHead;
                        continue;
                    }
                ...
                InsertionMode::AfterHead =>
                    {
                        ...
                        self.insert_elemnt("body", Vec::new());
                        self.mode = InsertionMode::InBody;
                        continue;
                    }
